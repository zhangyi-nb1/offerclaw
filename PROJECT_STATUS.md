...
|---|---|
| 身份 / 规则 / 边界层 | ✓ 完成 |
| 用户画像层 | ✓ 完成（§4 已填 Agent Demo 首条；§11 缺口由 2026-04-21 JD #6 匹配自动写入）|
| 岗位匹配层（双通路） | ✓ 完成 · LLM 版 + 规则版都通过 JD #6 端到端回归 |
| 路线规划层 | ✓ 完成 · `plan_prompt.md` + `plan_gen.py` 落地，已生成 04-22→04-28 周计划写入 daily_log |
| 执行推进 / 提醒层 | 🟡 90% · daily_log 模板就位 + JVS Claw 3 条定时任务已配置开启 |
| 学习留痕层 | ✓ 完成 · `summary_prompt.md` 9 步契约已落地，复盘任务由定时器驱动 |
| 代码实现层（Agent Demo） | ✓ **80%** · 加 `simple_profile_lookup` 工具 + 跨会话 `memory.json` + `/clear /save /history` 命令 |
| JVS Claw 部署 | 🟡 90% · 智能体 `jarvis` 跑通 onboarding / match / plan，3 条定时任务已开 |
| GitHub 公开化 | 🟡 50% · 仓库已建（2026-04-21），缺 .py 文件、缺 README 项目级、缺 PROJECT_STATUS |
| 简历 / 项目栏 | ⏳ 0% · 阶段 C 任务 |
...
按"功能层级"分组列出工作目录 `c:\Users\13513\Desktop\XIAGMU\` 下的全部 15 个文件（不含历史参考文档）。
...
### P1 · Day 2（2026-04-16）· 后台搭建视角

> **身份重合提示**：Day 2 的 Agent Demo 是"后台搭建"和"前台用户学习计划"两个身份的重合点。下面每条列的是**后台搭建产出**（"让 OfferClaw 项目多一个可交付组件"）。同一个动作从前台用户视角是"补项目"学习任务，应由 **daily_log.md 的 2026-04-16 条目**独立记录，用 `[补项目]` / `[补技能]` 主线标签——**本文档不写那些**。

- [ ] 扩展 `day1_api_starter.py` 成最小 Agent Demo：工具调用（`get_current_time` + 简单计算器）+ 多轮对话 + README
- [ ] Agent Demo 配套的 README 草稿（作为项目可交付物的一部分）
- [ ] 可选：day1_api_starter.py 的 temperature 扩展实验（改 0.0 vs 1.5 各跑一次对比，是 LLM 参数调研的后台验证）

### P2 · Day 3（2026-04-17）
- [ ] JVS Claw 首次部署：上传全部 MD 文件 + 配置 system prompt + 跑一次 onboarding + 跑一次 /match 指令
- [ ] 记录 JVS Claw 的实际 UI 行为给 Claude（文件引用语法、定时任务配置、工具接入方式）

### P3 · Day 4-6 · 规划模块 + 小修清理
- [ ] 新建 plan_prompt.md 或 plan_gen.py：从缺口清单 → 4 周计划（周/日两层 + 主线标签）
- [ ] 清理 user_profile.md §3 / §9 / §10 残留的 `【】` 占位符
- [ ] 把 jd_candidates.md 里的 JD #1–#5（字节的 5 份）逐一跑一遍新契约匹配，验证规则稳定性

### P4 · Day 7-10 · Agent Demo V2 迭代
- [ ] 加 RAG 小片段（把 jd_candidates.md 或 user_profile.md 作为知识库，让 Demo 能回答"我适合哪份实习"）
- [ ] 引入 LangGraph 重构工具调用循环（JD #6 要求的"优先项"技能）
- [ ] Demo 正式写入简历

### P5 · Day 11-16 · 收尾 + 投递
- [ ] 比赛材料：架构图 + 演示视频 + README
- [ ] 简历 V1 定稿
- [ ] 挑 1-2 份"当前适合投递"或"短期可投"的 JD 做实际投递
...
1. **时间紧**：距 2026-05-01 还有 16 天，核心价值链（画像-匹配-规划-执行-复盘）里规划 / 执行 / 复盘三个环节尚未真正落地
2. **项目经历 §4 是全链路的瓶颈**：所有 JD 匹配都卡在这一条；Day 2 Agent Demo 是解锁所有下游的关键依赖
3. **单源 JD 池**：jd_candidates.md 里 5/7 是字节，Day 3 横向对比前建议至少再补 1-2 份其他公司，避免规则对字节文风过拟合
4. **JVS Claw 细节未知**：部署流程是未知数，Day 3 可能需要多次来回调试

...
- **状态更新时间**：2026-04-25
- **版本**：**V2 · 全部 8 阶段优化完成**（可信度收口 → 控制台化 → Agent 主动规划 → JD 发现 → 简历生成 → RAG 扩展 → 多 Persona 验证 → 最终收口，pytest 37/37）
...
| 工程自检 | ✅ doctor 10 OK（auto-load `.env.local`，KEY 不入 git）· verify_pipeline 6/6 · verify_docs all green |
...
## V1.5 总完成度（历史参考，当前已进入 V2）

原 V1.5 各模块已全部完成并升级至 V2。以下为历史状态记录，当前实际指标见上方 V2 收口表。
...
| **RAG 检索增强系统（V2）** | ✓ **完成** · `rag_agent.py` + `rag_graph.py`（LangGraph）+ `rag_ingest.py` + `rag_tools.py` · Recall@5=**0.96** · cross_doc=**1.00** · MRR=**0.67** · 160 chunks · 8 类 source_type |
| **FastAPI 接口层（V2）** | ✓ **完成** · `rag_api.py` **19 路由**（13 核心业务 + 6 辅助/系统，含 2 条 SSE 流式 + Playwright SPA）+ Swagger UI |
| JVS Claw 部署 | 🟡 90% · 智能体 `jarvis` 跑通 onboarding / match / plan，3 条定时任务已开 |
| GitHub 公开化 | 🟡 **80%** · 仓库已建，README + PROJECT_STATUS 已同步最新进展，.py 文件已推送 |
| 简历 / 项目栏 | ✅ **docs/resume_pitch.md V2 终版**（80 字版 + 3 行版 + JD 映射表 + docs/demo_script.md 1 分钟演示脚本） |
...
| `chroma_db/` | ChromaDB 持久化目录（**160 chunks**，8 类 source_type，`offerclaw_docs` 集合） | ✅ |
...
| 文档解析、向量检索 | ✅ `rag_ingest.py` + ChromaDB（**160 chunks**，8 类 source_type） |
| 调用链路编排 / LLM 应用工作流 | ✅ LangGraph 4 节点 StateGraph |
| RAG 相关评估方法 | ✅ Recall@5=**0.96** · cross_doc=**1.00** · MRR=**0.67** |
...
| 蔚来 JD 职责 | 实现状态 |
|---|---|
| 推进系统原型设计、接口开发和上线部署 | ✅ `rag_api.py` **19 路由**（13 核心业务 + 6 辅助/系统，含 2 条 SSE 流式 + Swagger UI） |
...
| 文件 | 作用 | 完成度 |
|---|---|---|
| [rag_ingest.py](rag_ingest.py) | 文档向量化入库：Markdown → 分块 → Embedding → ChromaDB，支持 8 类 source_type | 100% |
...
| [rag_api.py](rag_api.py) | **FastAPI HTTP 接口层**（**19 路由**：13 核心业务 + 6 辅助/系统，含 2 条 SSE + Playwright + Swagger UI） | 100% |
| [rag_query.py](rag_query.py) | 独立检索查询脚本 | 100% |
| [career_agent.py](career_agent.py) | **全局求职 Orchestrator**（今日建议 / 缺口追踪，驱动 `/api/today`） | 100% |
| [job_discovery.py](job_discovery.py) | **JD 发现与抽取**（Playwright 无头渲染，支持 SPA 招聘页） | 100% |
| [resume_builder.py](resume_builder.py) | **简历草稿生成**（profile + story + JD → 定制 Markdown 段） | 100% |
| `chroma_db/` | ChromaDB 持久化目录（**160 chunks**，8 类 source_type，`.gitignore` 排除） | ✅ 运行中 |
...
### 第九批 · RAG + LangGraph + FastAPI V1.5 → V2（2026-04-22 ~ 04-30）
- **`rag_ingest.py` + `rag_tools.py`**：文档入库链路（RecursiveCharacterTextSplitter 分块 → 智谱 `embedding-3` 向量化 → ChromaDB 持久化）；8 类 source_type；**160 chunks**
...
- [x] RAG 系统（rag_agent + rag_graph + rag_ingest + rag_tools，**160 chunks**，Recall@5=0.96）
...
- [x] doctor.py 10 OK（auto-load `.env.local`）· verify_pipeline.py 6/6 · verify_docs.py all green · normalize_applications.py 0 error
...
- [x] 8 阶段 Career-Ops 优化全部完成

### 可选后续（V2 之后）

- [ ] 最终 PDF/Word 简历（从 resume_pitch.md 复制粘贴到模板）
- [ ] Demo 视频（可选，用于面试"扫码看"）
- [ ] RAG 评估集扩到 100 题（当前 50 题）

---

## 已知技术债务（可选优化，不影响投递）

1. **target_rules.md §1 方向硬编码**：当前写死了 Agent/AI/Prompt 三条，未来支持多用户时应参数化
2. **match_job.py DEMO_PROFILE 是字典镜像**：未来多用户需要真正的 Markdown 解析器
3. **match_job.py 关键词列表维护成本高**：可拆到独立 config 文件
4. **规则版项目契合度只判"有/无项目"**：语义匹配留给后续版本
5. **RAG 评估集当前 50 题**：可扩到 100 题（不影响当前 MRR=0.67 结论）
...
1. ~~时间紧~~：已在 05-01 前完成 V2 全部功能
2. ~~RAG 评估数据有限~~：Recall@5 从 0.75 提升至 **0.96**，MRR 从 0.69 提升至 **0.67**（160 chunks）
3. ~~FastAPI /api/match 仍是占位~~：已完整实现 19 路由

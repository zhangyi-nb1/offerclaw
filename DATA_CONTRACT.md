...
|---|---|---|---|---|---|---|
| `user_profile.md` | ✅ | ✅ | ❌ | ✅ | ⚠️ demo 入；真实建议 `*.example.md` | 仅追加；§0/§1 事实字段不得静默改 |
| `daily_log.md` | ✅ | ✅ | ✅ 仅追加 | ⚠️ 系统改动留"【系统更新】" | ✅ 脱敏 | 不可回溯改写 |
| `applications.md` | ✅ | ✅ | ✅ 仅追加新行（状态=已评估） | ✅ 状态前进必须确认 | ⚠️ 模板入；真实数据按需 | 字段见 §4.2；状态必须在合法枚举 |
| `interview_story_bank.md` | ✅ | ✅ | ⚠️ 仅生成草稿 | ✅ 用户合并 | ✅ | 每条须含 STAR+R + Metadata |
| `jd_candidates.md` | ✅ | ✅ | ✅ 追加摘录 | ❌ | ✅ | 测试池，非投递池 |
| `memory.json` | ✅ | ✅ | ✅ | ❌ | ❌ .gitignore | Agent 长期记忆 KV |
| `chroma_db/` | ✅ | ✅ | ✅ | ❌ | ❌ .gitignore | 本地向量库，~3.4MB |
| `.env.local` | ✅ | ✅ | ❌ | ✅ | ❌ .gitignore | 含智谱 API Key |
| `plans/` `summaries/` `logs/` | ✅ | ✅ | ✅ | ❌ | ❌ .gitignore | 运行时产物 |
| `profiles/p*.json` | ✅ | ✅ | ❌ | ✅ | ✅ | 仅作回归测试样本 |

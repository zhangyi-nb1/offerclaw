...
🔢 当前指标：**RAG Recall@5 = 0.96 · MRR = 0.68 · pytest 37/37 (+3 e2e skip) · 17 FastAPI 接口（含 SSE 流式 + 友好 UI + 6 卡片控制台 + 今日建议横条 + JD 半自动抽取 + JD 定制简历）· 138 chunks 知识库（含投递闭环 5 文档）· doctor 8 OK · verify_pipeline 6/6**
...
| **FastAPI 接口层** | ✅ | `rag_api.py`（17 路由：11 核心业务 + 6 辅助/系统，含 SSE 流式 + Swagger + 6 卡片控制台 + 今日建议 + JD 半自动抽取 + JD 定制简历） |
...
### 17 路由分类
- **核心业务 11**：`POST /api/match`（结构化 gap_list）· `POST /api/query` · `POST /api/stream` · `POST /api/search` · `POST /api/plan`（4 周路线）· `GET/POST /api/daily`（日志读写）· `GET /api/resume`（简历素材聚合）· `GET /api/today`（今日建议 Orchestrator）· `POST /api/discover`（JD 半自动抽取）· `POST /api/resume/build`（JD 定制项目段）· `POST /api/reset`

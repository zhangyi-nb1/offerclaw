...
🔢 当前指标：**RAG Recall@5 = 0.96 · MRR = 0.74 · pytest 37/37 (+3 e2e skip) · 14 FastAPI 接口（含 SSE 流式 + 友好 UI + 6 卡片控制台全联通）· 118 chunks 知识库 · doctor 8 OK · verify_pipeline 6/6**
...
| **FastAPI 接口层** | ✅ | `rag_api.py`（14 路由：8 核心业务 + 6 辅助/系统，含 SSE 流式 + 友好 UI + Swagger + 6 卡片控制台 4/5/6 卡全联通） |
...
### 14 路由分类
- **核心业务 8**：`POST /api/match`（结构化 gap_list）· `POST /api/query` · `POST /api/stream` · `POST /api/search` · `POST /api/plan`（4 周路线）· `GET/POST /api/daily`（日志读写）· `GET /api/resume`（简历素材聚合）· `POST /api/reset`

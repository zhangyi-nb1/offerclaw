...
### Result
- 35 个文件、17 个测试、11 个 FastAPI 接口、Recall@5 = 0.75 / MRR = 0.69
...
### Action
- 用 `rag_ingest.py` 把 9 份 markdown 切成 50 个 chunks（按 markdown header 切，不破坏语义单元）
- ChromaDB + 智谱 `embedding-3` 做向量检索
- 写 `eval_rag.py`：手工标注 8 题黄金集，跑 Recall@5 / MRR
- 调 chunk 大小（200/400/600 三档），最终选 400 token，Recall@5 = 0.75

### Result
"硬否决规则是什么" / "三档结论怎么定义" 这类元问题，准确率从 30% 提到 75%；LLM 输出可追溯到具体 chunk。

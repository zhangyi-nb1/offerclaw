...
----------------------------------------------------------------
V2 note: 118-chunk 基线 overall MRR=0.743；扩展到 160 chunks 后 MRR=0.67（精确值 0.673），
...
| 06 | Swagger 24 路由 (/docs) | 浏览器 |
| 07 | `/ui/console` 8 步 Stepper（CareerFlow 跑完一份真实 JD） | 浏览器 |

---

## 8. V3 状态真实化最终核验（2026-04-30）

> 本节回应：「`/api/match` 是否仍依赖 `match_job.DEMO_PROFILE`？CareerFlow / resume / jd rank 是否都基于真实 profile？」

### 8.1 全仓库 `DEMO_PROFILE` 引用清单

| 文件 | 性质 | 是否真实引用 | 处置 |
|---|---|---|---|
| `match_job.py` | 测试用 fixture（运行示例 + 单测数据源） | ✅ 真实定义 | **保留**（仅作 unit test fixture） |
| `tests/test_offerclaw_core.py` | 单元测试 | ✅ 真实使用 | **保留**（测试本就要钉死输入） |
| `tests/test_profile_loader.py` | 静态扫描守卫 | ✅ 用作 negative pattern | **保留**（守卫 `rag_api.py` 不再引用） |
| `pipeline.py` | CLI 端到端流水线 | ⚠️ 原引用 → **本轮已改为 `load_profile()`** | 已修复 |
| `career_flow.py` | 注释字符串 | ❌ 仅 docstring 提及，无代码引用 | 无需改 |
| `profile_loader.py` | 模块 docstring | ❌ 仅文档提及，无代码引用 | 无需改 |
| `rag_api.py` | FastAPI 主进程 | ❌ **0 引用** | ✅ 已彻底解耦（`/api/match`、`/api/flow/run`、`/api/jd/rank`、`/api/resume/markdown` 全部走 `load_profile()` 或下游模块）|

**生产模块真实引用数：0**（仅有 2 处 docstring 提及，无 import / 无函数调用）。

### 8.2 `/api/match` 真实 profile 行为校验

通过 `fastapi.testclient` 在进程内调用：

```text
POST /api/match  body={"jd_text":"岗位：AI Agent 实习生\n职责：基于 LangGraph 构建 RAG\n要求：Python，FastAPI，了解大模型","jd_title":"audit"}

[/api/match] HTTP 200
  status   : 中长期可转向
  direction: 主方向
  gap keys : ['硬门槛缺口', '技能缺口', '经历缺口']
```

`rag_api.py:418` 直接 `from profile_loader import load_profile; profile = load_profile(); report = run_match(profile, ...)` — 无 `DEMO_PROFILE` 兜底。

### 8.3 V3 四个核心 API → 状态源对照表

| API | 入参 | profile / state 源 | 是否过 LLM |
|---|---|---|---|
| `POST /api/match` | `jd_text` | `profile_loader.load_profile()` → `user_profile.md` | 否（纯规则） |
| `POST /api/flow/run` | `jd_text` | `career_flow.run_career_flow` → 内部同样走 `load_profile()` | 默认 `skip_llm=True`，零 LLM |
| `POST /api/jd/rank` | `candidates[]` | `job_discovery.rank_candidates` 内部对每条 JD 调用 `run_match(load_profile(), …)` | 否 |
| `POST /api/resume/markdown` | `jd_text` | `resume_builder.build_resume_markdown` 读 `user_profile.md` 段落 | 默认 `skip_llm=True`，零 LLM |

### 8.4 真实公网 JD 端到端验证

- 来源：[蔚来 NIO · 大模型应用开发实习生（VAS）](https://nio.jobs.feishu.cn/index/m/position/7249585265663641912/detail)（official · 可信度 A）
- 抓取规模：677 字符（Playwright 渲染兜底）
- CareerFlow 8 节点：`profile → job_input → match → gap → plan → today → resume → application_suggest`，**全过 0 错误**
- 结论：`status=当前适合投递`、`direction=主方向`、`requires_confirmation=1 patch`（不自动落 `applications.md`）
- 详细落证：[`docs/real_jd_run_nio_vas.md`](real_jd_run_nio_vas.md)

### 8.5 结论

- ✅ 生产 API 全部状态真实化，**无任何 DEMO_PROFILE 风险**
- ✅ 符合《产品级 Agent 化指导》§5 状态真实化验收标准（`/api/match` 不再使用 `DEMO_PROFILE`、CareerFlow 主流程零落盘、`requires_confirmation` 收口写意图）
- ✅ 真实公网 JD 已端到端跑通，**可以进入真实投递回流阶段**（下一步：在 `/ui/console` 走完一遍真投递、用户确认后由前端写回 `applications.md`）
